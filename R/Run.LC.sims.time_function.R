  ...
  for (s in 1:length(sens_vec)) {
    for (c in 1:length(spec_vec)) {
        for (r in 1:length(p_performed_vec)) {
          ...
          result_name <- paste("sens_", sens_vec[s], "_spec_", spec_vec[c], "_p_", p_performed_vec[r], sep = "")
          ...
          sim_data_df_ALL <- rbind(sim_data_df_ALL, sim_data$sim_data)
          ...
          # Assume this is within a loop with result_name and sim_data returned from sim.test.data.time()
          # Example: result_name <- "Sim1"; sim_data <- sim.test.data.time(...)
          ...
          # Ensure all have the model_id and standardize column types
          ...
          R_est_df_ALL <- rbind(R_est_df_ALL, R_est_df)


          #clear environment after each model run, but keeps neccessary bits
          required_vars <- c("num_tests", "sens_vec", "spec_vec", "p_performed_vec",
                             "sim_size", "iter", "chains", "warmup", "s", "c", "p", "r",
                             "data_ID", "prior_spec", "prior_sens", "seed", "stan_arg",
                             "sim_inputs_df_ALL", "sim_data_df_ALL",
                             "sim_stan_result_df_ALL", "sim_stan_result_df_ALL_long",
                             "divergence_summary_df",
                             "days",
                             "Est_R_window", "Est_R_n_samples", "mean_gi", "max_t",
                             "years", "N", "init", "SIR_params",
                             "R_est_df_ALL")
...
#' @title Runs multiple latent class model simulations over time with different parameter values in parallel.
#' @description Runs multiple simulations of a bayesian LC stan model to infer true disease prevalence over time for a seasonal disease, test sensitivity, and specificity from
#' simulated test data with different parameters values (sensitivity, specificity, and probability of the test occurring).
#' Can simulate multiple test results but all with identical parameters.
...
#' @name run.sims.LC.time.parallel
#' @examples
#' if (interactive()) {
#'  results <- run.sims.LC.time.parallel(num_tests = 4,
#'                        spec_vec = c(1),
#'                        sens_vec = c(1),
#'                        p_performed_vec= c(1),
#'                        sim_size = 100,
...
#' @export run.sims.LC.time.parallel
run.sims.LC.time.parallel <- function(num_tests, days = 365, spec_vec= c(1), sens_vec= c(1), p_performed_vec= c(1),
                                      sim_size=1000, iter=1000, chains=4, warmup=500, stan_arg=list(), data_ID = "sims",
                                      prior_spec = NULL, prior_sens = NULL, seed = 953,
                                      Est_R_window = 14, Est_R_n_samples = 1000, #for my R method
                                      mean_gi = NULL, max_t = NULL, #For EpiEstim - Generation interval (mean and SD)
