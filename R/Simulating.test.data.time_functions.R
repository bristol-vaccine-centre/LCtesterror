...
run.sir.model <- function(years = 50, N = 1, init = list(init_S = 0.99,init_I = 0.01,init_R = 0),
  ...
  if (is.null(params$beta0)) {
    params$beta0 <- params$desired_R0 * params$gamma
  }
  ...
  # Initial values
  init <- c(S = init$init_S, I = init$init_I, R = init$init_R)
  # Parameters
  parameters <- c(beta0 = params$beta0, beta1 = params$beta1, phi = pi/params$phi, gamma = params$gamma, omega = params$omega)
  #total population
  N <- N
  # Time points
  years <- years
  ...
  # Function to calculate Re(t) of SIR data
  calculate.Re.t <- function(R0_t, S, N) {
  ...
  # Calculate R0(t) for each time point
  output$R0_t <- calculate.R0.t(time = output$time, parameters = parameters)

  # Calculate Re(t) for each time point
  output$Re_t <- calculate.Re.t(R0_t = output$R0_t, S = output$S, N=N)

  #calculate beta_t
  output$beta_t <- calculate.beta_t(time = output$time, parameters = parameters)


  # Calculate proportion of infected individuals
  output$proportion_I <- output$I / (output$S + output$I + output$R)
  output$proportion_R <- output$R / (output$S + output$I + output$R)
  output$proportion_S <- output$S / (output$S + output$I + output$R)
  output$time_in_days <- output$time
  output$calculated_N <- output$S + output$I + output$R
  # Convert time to years
  output$time_in_years <- output$time / 365

  #warning if S+I+R population drifts from given N at any timepoints
  if (any(abs(output$calculated_N - N) > 1e-6)) {
    warning("Population size N does not match S + I + R at some time points!")
  }
...
sim.test.data.time <- function(sim_size = 1000, days = 365, test_params, seed=953,
 ...
 #save results
  ...
  results$SIR_params <- list(beta0 = params$beta0, desired_R0=params$desired_R0, beta1=params$beta1, phi=params$phi,
                             gamma=params$gamma, omega=params$omega, years=years, N=N,
                             init_S=init$init_S, init_I= init$init_I, init_R=init$init_R,
